...
$(IMAGES)/myos.flp: $(BIN)/first_stage.bin $(BIN)/second_stage.bin $(BIN)/third_stage.bin database.txt
	$(CAT) $(BIN)/first_stage.bin $(BIN)/second_stage.bin $(BIN)/third_stage.bin database.txt /dev/zero | $(DD) bs=512 count=2880 of=$(IMAGES)/myos.flp
