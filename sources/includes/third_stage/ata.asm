...
ata_print_info:
    pushaq
        ...
        mov rsi,comma        
        call video_print        
        mov byte [ata_identify_buffer+53],0x0    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;53   
...
ata_identify_disk:              ; rdi = channel, rsi = master/slave
    pushaq
        ...
        jl .error       ; Error if status is less than 2
         ...
         call video_print            
         jmp .out        
         .ready: ; R e a d f r o m b a s e p o r t 2 5 6 w o r d s  A T A I d e n tif y C o n fig u r a tio n D a t a →            
       ...
       ;  mov si,word [ata_identify_buffer_index]            
         ...
         xor rbx,rbx            
         rep insw            
         add word [ata_identify_buffer_index],256           ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;512 
         mov rsi, comma 
         call video_print 
       ;  call ata_print_size 
       ;  ata_print_size
...
read_disk_sectors: ;The purpose of this function is to read the disk sectors starting at sector 137 that will contain the database of pci devices
    pushaq
...
;selecting the disk to read from 
   
    mov r15, 0 ;This is the first sector to read in disk

    mov al, 0x2 ;disabling interrupts
    xor rdx, rdx
    mov dx, [ata_base_io_ports+rdi] 
    add dx, ATA_REG_COMMAND
    ;out dx, al

    mov rax,r15;extracting the first 8 bits of the address
    ...
    mov rax,r15;extracting the second 8 bits of the address
    shr rax, 8
    and rax,  0x00000000000000FF
    xor rdx, rdx
    mov dx, [ata_base_io_ports+rdi]
    ...
    mov rax,r15;extracting the third 8 bits of the address
    shr rax, 16
    and rax,  0x00000000000000FF
    xor rdx, rdx
    mov dx, [ata_base_io_ports+rdi]
    ...
    mov rax,r15;extracting the fourth 4 bits of the address
    shr rax, 24
    and rax, 0x000000000000000F
    or  rax, 0xe0 ;choosng the master
    xor rdx, rdx
    mov dx, [ata_base_io_ports+rdi]
    ...
    mov dx, [ata_base_io_ports+rdi]
    add dx, ATA_REG_SECCOUNT0
    mov ax, 255 ;number of sectors to read
    out dx, ax

    mov dx, [ata_base_io_ports+rdi]
    add dx, ATA_REG_COMMAND
    mov al, ATA_CMD_READ_PIO ;initiating read commands
    out dx, al
...
.check_ready:              ;loops until all sectors are read from disk   
         mov dx, [ata_base_io_ports+rdi]          
         add dx,ATA_REG_STATUS            
         in al, dx            
         xor rcx,rcx            
         ...
         mov rcx, 256            
         xor rbx,rbx            
         rep insw        ;This is a loop to read the first 256 character in the buffer
         .out:
         mov byte[test_string+255], 0
         mov rsi, test_string
         call video_print
